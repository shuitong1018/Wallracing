...
void Path_Identify(void) {
    ...
    // 状态分支 3：只有在普通巡线状态下，才允许捕捉并切入新路径！
        ...
        // 必须在没有冷却的情况下，才允许捕捉环岛入口
            ...
            // 【右环岛高速大放水】
            if (L1_norm > 70 && L4_norm > 100 && L1_norm < L4_norm && true_gyro_z > -400.0f && true_gyro_z <400.0f && true_gyro_x > -500.0f && true_gyro_x < 500.0f) {
                current_path_type = PATH_ROUNDABOUT;
                round_step = ROUND_FOUND;
                round_direction = 1;  // 右环岛
            }
            // 【左环岛同步升级】
            else if (L1_norm > 90 && L4_norm > 60 && L1_norm > L4_norm && true_gyro_z > -400.0f && true_gyro_z <400.0f  && true_gyro_x > -500.0f && true_gyro_x < 500.0f) {
                current_path_type = PATH_ROUNDABOUT;
                round_step = ROUND_FOUND;
