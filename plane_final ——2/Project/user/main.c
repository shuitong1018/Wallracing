...
void Gyro_Init_And_Calibrate(void) {
    ...
    for(i = 0; i < 200; i++) {
        ...
        offset_sum_x += imu660rb_gyro_x; // 【新增】累加x轴
        system_delay_ms(2);
    }
    gyro_z_offset = (float)offset_sum_z / 200.0f;
    gyro_x_offset = (float)offset_sum_x / 200.0f; // 【新增】算出x轴零偏
...
void patrol_line() {
    ...
    if (current_path_type == PATH_CROSS) {
        ...
        if (round_step == ROUND_FOUND) {
            ...
            if (round_direction == 1 && action_timer > 10) {  //右环岛
                round_step = ROUND_IN; 
                angle_z = 0; // 【关键】开始转弯前，把角度清零！
            } 
            else if (round_direction == -1 && action_timer > 14) {  //左环岛
                round_step = ROUND_IN; 
                angle_z = 0; // 【关键】开始转弯前，把角度清零！
            }
        }
        else if (round_step == ROUND_IN) {
            if (round_direction == 1) {  //右环岛
                dir_output = 1.4f; 
            ...
            dir_calculate();
            dir_output -= (float)round_direction * 0.4f; 
